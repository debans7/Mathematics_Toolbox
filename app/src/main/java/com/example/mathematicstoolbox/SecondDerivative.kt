...
class SecondDerivative : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_second_derivative)
        ...
        val button = findViewById<Button>(R.id.button).setOnClickListener {
            if(!notFilled(function,x)){
                ...
                Toast.makeText(this@SecondDerivative, "Please fill appropriate information", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun solve(function: EditText, X: EditText, H: EditText) {
        ...
        val fx : Double = exp.setVariable("x",x).evaluate()
        val ans ="%.${3}f".format((xn+xp-(2*fx))/(h*h))

        val intent = Intent(this,SecondDerivativeSolution::class.java).apply{
    ...
    private fun notFilled(function: EditText, x: EditText): Boolean{
